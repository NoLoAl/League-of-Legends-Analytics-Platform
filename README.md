...
Интерактивный дашборд для анализа матчей League of Legends, построенный на Python с использованием Dash, Plotly и DuckDB.
# Возможности
```
Фильтры
Регион — americas / europe (мультивыбор)
Роль — TOP, JUNGLE, MIDDLE, BOTTOM, UTILITY (мультивыбор)
Результат — Победа, Поражение, Все
```
# Вкладки

1. Общий срез и KPI
```
Количество матчей — числовой индикатор
Среднее KDA — числовой индикатор
Среднее время матча — числовой индикатор (в минутах)
Топ-15 чемпионов по популярности — горизонтальная столбчатая диаграмма
Топ-15 чемпионов по винрейту — горизонтальная столбчатая диаграмма
Распределение длительности матчей — гистограмма с box-plot
Распределение игроков по LP — гистограмма с box-plot
```
2. Герои и Роли
```
Связь убийств и золота — scatter-диаграмма по чемпионам и ролям (размер точки = количество игр)
Топ-10 предметов у победивших — столбчатая диаграмма
Топ-5 предметов по ролям — столбчатая диаграмма с цветовым разделением по предметам
```
3. Статистика
```
Распределение KDA по ролям — box plot с цветовой дифференциацией ролей
Корреляционная матрица — heatmap ключевых игровых метрик (kills, deaths, assists, gold, damage, minions, vision_score, KDA)
Сводная таблица по чемпионам — DataTable с показателями: игры, винрейт, средний KDA, среднее золото, средний урон
```
---
